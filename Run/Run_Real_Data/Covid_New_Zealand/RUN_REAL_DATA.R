...
data_type = '2_nz_south_2020'
file_data = 'epi_data_nz2.rds'
epidemic_data = readRDS(file_data)

#data_type = 'nz_waita2_2021'
#file_data = 'epi_data_waita2.rds'
#epidemic_data = readRDS(file_data)

#data_type = 'nz_waita_2021'
#file_data = 'epi_data_waita.rds'
...
RUN_MODELS_REAL_DATA <- function(iteration, epidemic_data, n_mcmc = 62500){
  ...
  if(iteration == 1){
    ...
    saveRDS(mod_ev_ssib, file = file_name)
...
time_stp = GET_CURRENT_TIME_STAMP()
file_name = paste0('vec_mod_ev_', time_stp, '_', data_type, '.rds')
