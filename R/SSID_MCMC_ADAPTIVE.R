...
#LOG LIKELIHOOD
#**********************************************
LOG_LIKELIHOOD_SSID <- function(x, nu_params, eta){ #eta - a vector of length x. eta[1] = infectivity of xt[1]
...
MCMC_ADAPTIVE_SSID <- function(dataX, OUTER_FOLDER, seed_count,
  ...
  log_like_vec[1] <- LOG_LIKELIHOOD_SSID(dataX, nu_params, eta);  log_like = log_like_vec[1]
  ...
  for(i in 2:n_mcmc) {
    ...
    if (min(nu_params_dash - vec_min) >= 0){ 
      ...
      #LOG LIKELIHOOD
      logl_new = LOG_LIKELIHOOD_SSID(dataX, nu_params_dash, eta)
      #ACCEPTANCE RATIO
    ...
    for(t in which(dataX[1:(num_days-1)] > 0)){ #Only update eta's where x[t] > 0
      ...
      #LOG LIKELIHOOD
      logl_new = LOG_LIKELIHOOD_SSID(dataX, nu_params, eta_dash)
      log_accept_ratio = logl_new - log_like
