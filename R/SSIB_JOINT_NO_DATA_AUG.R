...
LOG_LIKE_SSIB_ND <- function(data, ssib_params, 
...
MCMC_INFER_SSIB_ND <- function(data, n_mcmc, PRIORS_USED = GET_PRIORS_USED(),
  ...
  log_like_vec[1] <- LOG_LIKE_SSIB_ND(data, ssib_params) #, FLAG_NEGBIN_PARAMATERISATION)
  ...
  for(i in 2:n_mcmc) {
    ...
    if (min(ssib_params_dash - vec_min) >= 0 && ssib_params_dash[2] < 1){ 
      ...
      #LOG LIKELIHOOD
      logl_new = LOG_LIKE_SSIB_ND(data, ssib_params_dash) 
