...
#1. LOG LIKELIHOOD
LOG_LIKE_SSEB <- function(x, lambda_vec, alphaX, r0, gammaX){
  
  #Params
  num_days = length(x); logl = 0
  
  betaX = (r0 - alphaX)/gammaX
...
MCMC_INFER_SSEB <- function(epidemic_data, n_mcmc = 30000,
  ...
  if (FLAGS_LIST$ADAPTIVE){
    ...
    #sigma_bg_vec <- vector('numeric', mcmc_vec_size) sigma_ag_vec <- vector('numeric', mcmc_vec_size);
  ...
  for(i in 2:n_mcmc) {
    ...
    while(alpha_dash < 0 || alpha_dash > r0){
      
      if (alpha_dash > r0){
        alpha_dash = 2*r0 - alpha_dash
      }
    ...
    if (FLAGS_LIST$PRIOR){
      ...
      dbeta(alpha_dash/r0, 1, 2, log = TRUE) - dbeta(alpha/r0, 1, 2, log = TRUE) #Jacobian cancels
      }
    ...
    #Priors
    if (FLAGS_LIST$r0_PRIOR_GA){
      log_accept_ratio = log_accept_ratio +
        dgamma(r0_dash, shape = priors_list$r0_prior_ga[1], scale = priors_list$r0_prior_ga[2], log = TRUE) -
        dgamma(betr0_dasha, shape = priors_list$r0_prior_ga[1], scale = priors_list$r0_prior_ga[2], log = TRUE)
    
      } else {
        log_accept_ratio = log_accept_ratio - r0_dash + r0
    }
    ...
    if(FLAGS_LIST$GAMMA_PRIOR_GA){
      #print('GAMMA prior on gamma')
      log_accept_ratio = log_accept_ratio + dgamma(gamma_dash - 1, shape = priors_list$gamma_prior_ga[1], scale = priors_list$gamma_prior_ga[2], log = TRUE) -
        dgamma(gamma - 1, shape = priors_list$gamma_prior_ga[1], scale = priors_list$gamma_prior_ga[2], log = TRUE)
    } else {
