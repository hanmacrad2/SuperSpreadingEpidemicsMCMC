...
#' @examples
#'
#' log_like = LOG_LIKE_SSIB_FIX_C(epidemic_data, 0.8, 0.02, 20)
#'
#' @export

LOG_LIKE_SSIB_FIX_C <- function(epidemic_data, alpha, r0, c = 10, 
...
#ll = LOG_LIKE_SSIB_FIX_C(vec, 0.1, 2, 10)
...
#************************************************************************
MCMC_INFER_SSIB_FIX_C <- function(epidemic_data, n_mcmc = 40000,
  ...
  log_like_vec[1] <- LOG_LIKE_SSIB_FIX_C(data, alpha_vec[1], r0_vec[1])
  ...
  for(i in 2:n_mcmc) {
    ...
    logl_new = LOG_LIKE_SSIB_FIX_C(data, alpha_dash, r0) #RO 
    ...
    #loglikelihood
    logl_new = LOG_LIKE_SSIB_FIX_C(data, alpha, r0_dash)
    ...
    for(t in 1:time){
      ...
      if((data_dash[[2]][t] < 0) || (data_dash[[1]][t] < 0)){
        ...
        logl_new = LOG_LIKE_SSIB_FIX_C(data_dash, alpha, r0) #+include alpha prior 
        log_accept_ratio = logl_new - log_like
      }
    ...
    #if (!(log_like && LOG_LIKE_SSIB_FIX_C(data, a, b, c))) print('loglike doesnt exist')
    #else (log_like!=LOG_LIKE_SSIB_FIX_C(data, a, b, c)) print(paste0('ERROR! logl diff = ', log_like - LOG_LIKE_SSIB_FIX_C(data, a, b, c)))
