...
#' MCMC adaptive algorithm for Super-Spreading Individuals (SSI) epidemic model
#'
...
#'   \item \code{"sigma"} - A list of vectors containing the adapted sigma for each of the model parameters at each iteration of the MCMC. Vector size \code{"mcmc_vec_size"}
...
SSI_MCMC_ADAPTIVE <- function(data,
                              mcmc_inputs = list(n_mcmc = n_mcmc,
                                                 mod_start_points = mod_start_points, alpha_star = 0.4,
                                                 thinning_factor = 10),
  ...
  sigma = list(sigma1 <- vector('numeric', mcmc_vec_size), sigma2 <- vector('numeric', mcmc_vec_size), sigma3 <- vector('numeric', mcmc_vec_size),
               sigma4 <- vector('numeric', mcmc_vec_size), sigma5 <- vector('numeric', mcmc_vec_size))
