...
SIMULATE_SSIB_VI = function(a = 0.5, b = 0.1, c = 10,
  ...
  for (t in 2:num_days) {
    ...
    #Regular infections
    lambda_t = sum((total_infections[1:(t-1)] + c*ssi_infections[1:(t-1)])*rev(prob_infect[1:(t-1)])) #Product of infections & their probablilty of infection along the gamma dist at that point in time
    nssi_infections[t] = rpois(1, a*lambda_t) #Assuming number of cases each day follows a poisson distribution. Causes jumps in data 
    ssi_infections[t] = rpois(1, b*lambda_t)
...
#' @param a SSI model parameter \code{"a"}
#' @param b SSI model parameter \code{"b"}
...
#' @examples
#'
#' log_like = LOG_LIKE_SSIB_VI(epidemic_data, 0.8, 0.02, 20)
#'
LOG_LIKE_SSIB_VI <- function(sim_data, a, b, c){

  #Datawdf
  non_ss = sim_data[[1]]; ss = sim_data[[2]]
  ...
  for (t in 2:num_days) { #*1 or 2
    ...
    #loglike = loglike - lambda_t*(a + b) + n[t]*(log(a) + log(lambda_t)) + s[t]*(log(b) + log(lambda_t))  + 2*log(1) - lfactorial(n[t]) - lfactorial(s[t])
  }
...
#************************************************************************
MCMC_INFER_SSIB_VI <- function(epidemic_data, n_mcmc = 100000,
                              mcmc_inputs = list(mod_start_points = list(m1 = 0.5, m2 = 0.1, m3 = 10), alpha_star = 0.4,
                                                 thinning_factor = 10,  burn_in_pc = 0.2),
                              priors_list = list(a_prior_exp = c(1, 0), b_prior_ga = c(10, 2/100), b_prior_exp = c(0.1,0),
                                                 c_prior_ga = c(3, 3), c_prior_exp = c(0.1,0)),
                              FLAGS_LIST = list(ADAPTIVE = TRUE, DATA_AUG = TRUE, BCA_TRANSFORM = TRUE,
                                                PRIOR = TRUE, B_PRIOR_GAMMA = FALSE, C_PRIOR_GAMMA = TRUE,
                                                THIN = TRUE)) {
  ...
  #INITIALISE PARAMS
  #**********************************************
  time = length(epidemic_data[[1]]); 
  print(paste0('total num mcmc iters = ', n_mcmc))
  ...
  log_like_vec[1] <- LOG_LIKE_SSIB_VI(data, a_vec[1], b_vec[1], c_vec[1])
  ...
  vec_accept_da = vector('numeric', time)
  ...
  for(i in 2:n_mcmc) {
    ...
    #log a
    logl_new = LOG_LIKE_SSIB_VI(data, a_dash, b, c)
    ...
    #loglikelihood
    logl_new = LOG_LIKE_SSIB_VI(data, a, b_dash, c)
    log_accept_ratio = logl_new - log_like
    ...
    logl_new = LOG_LIKE_SSIB_VI(data, a, b, c_dash)
    log_accept_ratio = logl_new - log_like

    #Priors
    if(FLAGS_LIST$C_PRIOR_GAMMA){
      log_accept_ratio = log_accept_ratio + dgamma(c_dash, shape = priors_list$c_prior_ga[1], scale = priors_list$c_prior_ga[2], log = TRUE) -
    ...
    if(FLAGS_LIST$BCA_TRANSFORM){
      ...
      if(b_transform >= 0){ #Only accept values of b > 0
        ...
        logl_new = LOG_LIKE_SSIB_VI(data, a, b_transform, c_dash)
        log_accept_ratio = logl_new - log_like
    ...
    if(FLAGS_LIST$BCA_TRANSFORM){
      ...
      if(a_transform >= 0){ #Only accept values of b > 0
        ...
        logl_new = LOG_LIKE_SSIB_VI(data, a_transform, b, c_dash)
        log_accept_ratio = logl_new - log_like
    ...
    for(t in 1:time){
      ...
      if((data_dash[[2]][t] < 0) || (data_dash[[1]][t] < 0)){
        ...
        logl_new = LOG_LIKE_SSIB_VI(data_dash, a, b, c) #+include alpha prior 
        log_accept_ratio = logl_new - log_like
      }
    ...
    #if (!(log_like && LOG_LIKE_SSIB_VI(data, a, b, c))) print('loglike doesnt exist')
    #else (log_like!=LOG_LIKE_SSIB_VI(data, a, b, c)) print(paste0('ERROR! logl diff = ', log_like - LOG_LIKE_SSIB_VI(data, a, b, c)))

    #POPULATE VECTORS (ONLY STORE THINNED SAMPLE)
    #POPULATE VECTORS (ONLY STORE THINNED SAMPLE)
    if (i%%thinning_factor == 0 && i >= burn_in_start && i_thin < mcmc_vec_size) {
      a_vec[i_thin] <- a; r0_vec[i_thin] <- a + b*c
      b_vec[i_thin] <- b; c_vec[i_thin] <- c     
  ...
  list_accept_rates$vec_accept_da = vec_accept_da
