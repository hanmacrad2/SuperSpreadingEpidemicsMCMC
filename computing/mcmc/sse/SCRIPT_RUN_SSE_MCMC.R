...
GET_SSE_MCMC_ROW_K <- function(k, mcmc_output){
...
#k
#********
RUN_INFERENCE_SSE_K <- function(CURRENT_FOLDER, num_runs = 1000, R0 = 1.5, 
                              n_mcmc = 50000) {
  ...
  for (i in 1:length(list_k)){
    ...
    #Row result
    result_row = GET_SSE_MCMC_ROW_K(k, mcmc_output)
    df_results <- bind_rows(df_results, result_row)
...
#RUN_INFERENCE_SSE
RUN_INFERENCE_SSE_K(CURRENT_FOLDER)
